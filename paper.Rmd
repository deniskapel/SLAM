...
A task can contain from 1 to 14 tokens (depending on the language). All UD features were retrieved by @slam18 using the Google SyntaxNet dependency parser and the language-agnostic Universal Dependencies tagset^[Parse errors may occur.].
...
fr_en_sessions %>%
  filter(POS=='ADP') %>% 
  separate_rows(morph, sep = "\\|") %>%
  separate(morph, into = c("feature", "value"), sep = "=")
```
